...
\section{DPPs in RandNLA}

\begin{frame}
  \frametitle{DPPs in RandNLA}
  \underline{Given}: data matrix $\X$ \\[2mm]
...
\end{frame}
  ...
  \begin{frame}
    \frametitle{Subsampled least squares}
\centerline{\includegraphics[width=.6\textwidth]{figs/regression-subset}}
\vfill
$$\Blue{\hat \w}=\argmin_{w} \sum_{i\in S} (x_i w-y_i)^2$$
\end{frame}
...
\begin{frame}
  \frametitle{DPP sampling for least squares regression}
  \onslide<+->
  \begin{theorem}[\cite{unbiased-estimates}]
...
\section{DPPs in Experimental Design}
...
\begin{center}\fcolorbox{reddishyellow}{lightyellow}{
 \parbox{0.95\textwidth}{  
   \textbf{Theorem} (\cite{avron-boutsidis13,unbiased-estimates})\\
For any $k\geq d$, there is a DPP distribution over $S$ such that:
   $$\underbrace{\mathrm{MSE}[\X_S^\dagger\y_S]}_{\E[\sigma^2\tr((\X_S^\top\X_S)^{-1})]}\leq \frac{n-d+1}{k-d+1}\cdot \underbrace{\MSE{\X^\dagger\y}}_{\sigma^2\tr((\X^\top\X)^{-1})}$$
}}\end{center}
\end{frame}
...
\begin{center}\fcolorbox{reddishyellow}{lightyellow}{
 \parbox{0.95\textwidth}{  
  \textbf{Learn more:} \cite{dpps-in-randnla}
