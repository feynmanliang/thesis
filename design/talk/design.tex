...
\begin{frame}
\frametitle{In general: \textit{add dimension many points}}
\transfade<2,4->[duration=0.25]
...
\begin{frame}
  ...
  \item Augmentation uses a determinantal point process (DPP) we call \emph{volume-rescaled sampling}\\[4mm]
    \pause
  \item There are many \emph{efficient DPP algorithms} \hfill \Blue{(talk Thu at 9am)}\\[4mm]
    \pause
...
\vspace{-3mm}
\begin{align*}
\det(\X^\top\X - \x_i\x_i^\top) = \det(\X^\top\X)\big(1 - \overbrace{\x_i^\top(\X^\top\X)^{-1}\x_i}^{\text{$i$-th leverage score}}\big)
\end{align*}
\vspace{4mm}
\pause
...
Leverage score sum:
\begin{align*}
\sum_{i=1}^n \x_i^\top(\X^\top\X)^{-1}\x_i = \tr(\X(\X^\top\X)^{-1}\X^\top) = d
\end{align*}
\end{frame}
...
\sum_{i=1}^{d+1} 
\det\!\big(\X_{[d\!+\!1]}^{\sigma\top}\X_{[d\!+\!1]}^{\sigma}\big)\Red{\Big(1 - \x_{\sigma_i}^\top\!\big(\X_{[d\!+\!1]}^{\sigma\top}\X_{[d\!+\!1]}^{\sigma}\big)^{\!-1}
\!\x_{\sigma_i}\!\Big)}\!\bigg]\\}
\onslide<5->{
