...
\citep{Williams01Nystrom,revisiting-nystrom}.
Among others, this approximation error
appears in the analysis of sketched kernel
ridge regression \citep{fanuel2020diversity} and Gaussian process
regression \citep{sparse-variational-gp}. Furthermore, a variety of
iterative algorithms, such as randomized second-order methods for
convex optimization \citep{Qu2015Feb,Qu2016,Gower2019,jacsketch}
and linear system solvers based on
the generalized Kaczmarz method \citep{generalized-kaczmarz},
...
linear models \citep{surrogate-design,BLLT19_TR}.
...
\citep{hachem2007deterministic,bai2010spectral}. However, our analysis
...
\begin{enumerate}
  \item The residual projection matrix can be obtained
        ...
        \citep{HMRT19_TR}) combined this with an exchange-of-limits argument to
...
embeddings (e.g.,
\citep{sarlos-sketching,cw-sparse,nn-sparse,projection-cost-preserving,optimal-matrix-product}),
...
random variable \citep{achlioptas2003database}. We start by analyzing the Frobenius
...
Nystr\"om method \citep{revisiting-nystrom}. A Nystr\"om approximation
...
Component Analysis \citep{sparse-variational-gp,Williams01Nystrom,Bach2003}. By setting
$\A=\K^{\frac12}$, it is easy to see \citep{nystrom-multiple-descent} that
...
decay \citep{Santa97gaussianregression,RasmussenWilliams06}.

Metrics other than the aforementioned Frobenius norm error,
such as the spectral norm error \citep{tropp2011structure}, are also of significant
...
\citep{generalized-kaczmarz}. With
...
\citep{Gower2019,jacsketch} in \Cref{a:newton}.
...
regularization conjecture posed by \citep{surrogate-design} (see
...
\end{align*}
While implicit regularization has received attention recently in the context of SGD algorithms for overparameterized machine learning models, it was originally discussed in the context of approximation algorithms more generally~\citep{Mah12}.
Recent work has made precise this notion in the context of RandNLA \citep{surrogate-design}, and our results here can be viewed in terms of implicit regularization of scalable RandNLA methods.
...
Randomized Subspace Newton (RSN, \citep{Gower2019})  is a randomized
...
\citep{Gower2019} in Assumption 1, which also defines relative strong
...
Jacobian Sketching (JacSketch, \citep{jacsketch}) defines an $n\times n$ positive semi-definite weight matrix
...
\begin{proof}
  ...
  %for $c = (36 \sqrt{2})^2$ (details in proof of Theorem~3.2 in \citep{burkholder1973distribution}).
...
empirical results shown in \Cref{a:experiments}.
